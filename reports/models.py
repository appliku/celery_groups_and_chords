...
class ReportChord(models.Model):
    ...
    def do_celery(self):
        s_short = signature("report_chord_short", kwargs={"report_id": self.pk})
        s_medium = signature("report_chord_medium", kwargs={"report_id": self.pk})
        s_long = signature("report_chord_long", kwargs={"report_id": self.pk})
        tasks = [s_short, s_medium, s_long]
        s_error = signature("report_chord_error", kwargs={"report_id": self.pk}, immutable=True)
        s_finished = signature("report_chord_finished", kwargs={"report_id": self.pk}, immutable=True)
        s_finished.on_error(s_error)
